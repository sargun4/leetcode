...
public:
    int findCheapestPrice(int n, vector<vector<int>>& flights, int src, int dst, int k) {
        ...
        price[src] = 0; // The price to reach the src node from itself is 0
    
        queue<pair<int, pair<int, int>>> q; //q= {no. of stops, {curr node, curr cost}}
        q.push({0, {src, 0}}); // start with the src node, 0 stops, and cost 0

        while(!q.empty()) {
            auto p = q.front();
            int stop = p.first; // no of stops made to reach the curr node
            int node = p.second.first; // curr node
            int cost = p.second.second; // cost to reach the curr node
            q.pop();

            // if the no. of stops exceeds k, skip processing further
            if(stop > k) {
                continue;
            }

            for(auto it : adj[node]) {
                // if  no. of stops is within the limit and the new cost is less than the prev recorded cost
                if((stop <= k) && (cost + it.second)<price[it.first]) {
                    price[it.first] = cost + it.second; //update cost to reach adj node
                    q.push({stop + 1, {it.first, cost + it.second}}); // push adj node into q with updated cost and stops
                }
            }
        }
