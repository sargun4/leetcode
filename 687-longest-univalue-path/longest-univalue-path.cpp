...
// 2. The **global maximum** must be updated at every node based on the sum of its left and right same-value paths.
...
public:
    ...
    // DFS helper function: returns the longest univalue path (in terms of edges) 
    // starting from this node that can be extended by its parent.
