...
//                         // Remove the dependency and check if neighbor is ready
//                         if (--indegree[x][y] == 0)
//                             q.push({x,y});
//                     }
//                 }
...
public:
    ...
    int dfs(vector<vector<int>> &matrix, int i, int j, vector<vector<int>> &dp) {
        ...
        for(auto &d:dirns){
            int x=i+d[0];
            int y=j+d[1];
            //inbound n next cell vis shd be stricly greater than prev cell 
            if(isvalid(x,y,m,n) && matrix[x][y]>matrix[i][j]){
                maxlen=max(maxlen,1+dfs(matrix,x,y,dp));
            }
        }
        dp[i][j]=maxlen;
        return maxlen;
