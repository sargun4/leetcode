...
public:
    int maxCandies(vector<int>& status, vector<int>& candies, vector<vector<int>>& keys, vector<vector<int>>& containedBoxes, vector<int>& initialBoxes) {
        int ans=0;
        unordered_set<int> vis;//candies frm those boxes which hv been vis to not be considered again=>vis
        unordered_set<int> foundBoxes;//those boxes which hv been found =>keep a check for closed boxes which might get opende later on if we find the key in another box
        
        queue<int> q;//insert thsoe boxes which we hv now n are open
        for(int &box:initialBoxes){
            foundBoxes.insert(box);
            if(status[box]==1){
                q.push(box);
