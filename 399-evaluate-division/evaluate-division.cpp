...
public:
    ...
    //for each equation (a/b=k), add edges (a,b) & (b,a)to the adj with edge values k & 1/k, respectively
        unordered_map<string,unordered_map<string,double>> adj;//{num:{}}
        for(int i=0;i<equations.size();i++){
            string dividend=equations[i][0];
            string divisor=equations[i][1];
            double val=values[i];
            adj[dividend][divisor]=val;
            adj[divisor][dividend]=1/val;
        } 
        ...
        for (auto& query : queries) {
            ...
            if (adj.find(dividend) == adj.end() || adj.find(divisor) == adj.end()) {//if y isnt reached or either x or y is not present in adj, return -1.
                res.push_back(-1.0);
            } else {
